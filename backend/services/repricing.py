            ...
            for product in products:
                ...
                if not product.n11_url and product.sku:
                    ...
                    async def fetch_n11_search(query: str):
                        try:
                            # N11 aramalarında rate limit'e takılmamak için kısa bir bekleme
                            ...
                            if resp.status_code == 200:
                                ...
                                for script in soup.find_all('script'):
                                    if script.string and 'window.model = ' in script.string:
                                        ...
                                        idx = raw.find('window.model = ')
                                        start_idx = raw.find('{', idx)
                                        open_brackets = 0
                                        end_idx = -1
                                        for i in range(start_idx, len(raw)):
                                            if raw[i] == '{': open_brackets += 1
                                            elif raw[i] == '}':
                                                open_brackets -= 1
                                                if open_brackets == 0:
                                                    end_idx = i + 1
                    ...
                    if results:
                        ...
                        # Her halükarda kendi mağazamızı bulmaya çalış
                        for item in results:
                        ...
                        if not target_item:
                            if is_sku_search:
                                # SKU çok spesifiktir, eğer sonuç varsa ilkini alabiliriz
                                target_item = results[0]
                            else:
                                logger.info(f"[Repricing] İsim araması mağaza ({tenant.name}) ile eşleşmedi. Yanlış ürün (Honda vb) eklenmemesi için atlanıyor.")
                                target_item = None
