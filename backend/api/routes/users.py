...
@router.get("/me/fix-n11-urls")
async def fix_n11_urls(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    # Kullanıcının tüm ürünlerini çek ve magaza=saygingrup ekle
    result = await db.execute(select(Product).where(Product.tenant_id == current_user.tenant_id))
    products = result.scalars().all()
    count = 0
    for p in products:
        if p.n11_url and "magaza=" not in p.n11_url.lower():
            sep = "&" if "?" in p.n11_url else "?"
            p.n11_url = f"{p.n11_url}{sep}magaza=saygingrup"
            db.add(p)
            count += 1
    await db.commit()
    return {"message": f"{count} adet ürün linkine başarıyla magaza=saygingrup eklendi."}
